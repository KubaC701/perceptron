...
class DataManager:
  ...
  def generate_data_chart(self):
    data = self.__prepare_to_view()
    df = pd.DataFrame(data)
    df_plot = df.plot.scatter(
        x=CAR_HORSEPOWER_COLUMN,
        y=CAR_WEIGHT_COLUMN,
        c=df[EXPECTED_COLUMN].map(self.__map_colors)
    )
  ...
  def read_file(self, file_name):
    ...
    compressed_data = self.__compress_to_calculate(data)
  ...
  @staticmethod
  def __compress_to_calculate(data):
    new_data = data
  ...
  @staticmethod
  def generate_loss_chart(epoch_loss):
    df = pd.DataFrame(epoch_loss)
    df_plot = df.plot(kind='line', grid=True).get_figure()
    df_plot.savefig(TRAINING_LOSS_FILE_NAME)

  def __prepare_to_view(self):
